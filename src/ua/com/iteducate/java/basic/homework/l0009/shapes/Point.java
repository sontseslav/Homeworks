...
public class Point {
	...
	public double calcDist(Point pt2) throws ZeroDistanceException {
		//check if x=y ||y=y for optimization
		double distance = Math.sqrt(Math.pow(this.x-pt2.x,2)
				+Math.pow(this.y-pt2.y, 2));
		if (distance == 0) throw new ZeroDistanceException();
		return distance;
	}
